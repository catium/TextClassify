...
public class BasicTextClassifier{
	...
	public boolean addTrainingText(String category, String filename) {
		...
		try {
			readContentFromFileOrTable(filename, label);
		} catch (IOException e) {
	...
	//amber
	void readContentFromFileOrTable(String fileName, int label) throws IOException
	{
		String content;
		if(fileName.endsWith(".csv"))
		{
			...
			{
				content = trainerfilter(line);
				classifier.addTrainingText(line, label);
			}
		}
		else
		{

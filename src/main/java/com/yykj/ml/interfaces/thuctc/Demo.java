package com.yykj.ml.interfaces.thuctc;
...
public class Demo {
	...
	public static void runTrainAndTest() {
		...
		String defaultArguments = ""
			 + "-train D:\\MLSpace\\ZL "  // 设置您的训练路径，这里的路径只是给出样例
			 //+ "-test D:\\MLSpace\\THUCNews "
			...
			+  "-s D:\\MLSpace\\model"  // 将训练好的模型保存在硬盘上，便于以后测试或部署时直接读取模型，无需训练
			 ;
	 ...
	 * 
	 */
	public static void runLoadModelAndUse(String text) {
		// 新建分类器对象
		BasicTextClassifier classifier = new BasicTextClassifier();
		
		// 设置分类种类，并读取模型
		classifier.loadCategoryListFromFile("D:\\MLSpace\\model\\category");
		classifier.setTextClassifier(new LinearBigramChineseTextClassifier(classifier.getCategorySize()));
		classifier.getTextClassifier().loadModel("D:\\MLSpace\\model");
